...
def cargar_base_operativa():
    ...
    return {
        ...
        "renewals": renewals,
        "spreadsheet": sheet  # necesario para escribir cambios después
    }
...
def generar_archivo_revision(first_renewal_day):
    ...
    return buffer  # 👉 Se retorna como archivo descargable
...
def procesar_facturacion(revision_file, actualizacion_csv):
    ...
    return billing_df  # 👉 Puedes mostrarlo en Streamlit si quieres
...
def actualizar_activaciones(ventas_file):
    ...
    # === Paso 3: Actualizar hoja Day si hay nuevos códigos ===
...
def procesar_recargas_dia(fecha_deseada):
    ...
    # === Paso 2: Eliminar líneas canceladas (por msisdn o iccid) ===
    ...
    # === Paso 3: Filtrar las que se deben recargar ese día ===
...
def cancelar_linea(identificador):
    ...
    # === Función para buscar en una hoja ===
    def buscar(df):
        df = df.copy()
        df['msisdn'] = df['msisdn'].astype(str)
        df['iccid'] = df['iccid'].astype(str)
    ...
    for fuente in fuentes:
        ...
        if not encontrado.empty:
            registro = encontrado.iloc[[-1]]  # último match si hay varios
            break

    # === Procesar resultado ===
    if registro.empty:
        return {"status": "error", "mensaje": "❌ No se encontró el identificador."}
    else:
        ...
        return {"status": "ok", "mensaje": f"✅ Cancelación registrada para {nombre}."}
...
def reactivar_linea(msisdn_iccid, codigo_input, fecha_reactivacion_input):
    ...
    if not registro.empty:
        ...
        if codigo_input != codigo_encontrado:
            cambio = input(f"⚠️ El código ingresado ({codigo_input}) no coincide con el encontrado ({codigo_encontrado}). ¿Es cambio de DS? (s/n): ").strip().lower()
            if cambio != "s":
                raise Exception("❌ Registro cancelado por discrepancia en código de distribuidora.")
            codigo_distribuidora = codigo_input
        else:
            codigo_distribuidora = codigo_encontrado
    else:
        print("🔍 Línea nueva. Se solicitará al usuario la información.")
    ...
    if codigo_distribuidora not in asociados_df['codigoDistribuidora'].astype(str).values:
        id_asociado = input("Nuevo código detectado. Ingresa el ID del asociado: ").strip()
    ...
    if codigo_distribuidora not in day_df['codigoDistribuidora'].astype(str).values:
        print("Código no encontrado en hoja Day.")
        dias_validos = ["LUNES", "MARTES", "MIERCOLES", "JUEVES", "VIERNES"]
        while True:
            dia_fact = input("¿Qué día se factura esta DS? (Ej. LUNES, CAPS sin acentos): ").strip().upper()
    ...
    # === Paso 4: Calcular fecha de facturación con al menos 7 días de adelanto ===
    dias_semana = {
        "LUNES": 0, "MARTES": 1, "MIERCOLES": 2,
        "JUEVES": 3, "VIERNES": 4, "SABADO": 5, "DOMINGO": 6
    }
    ...
    # === Paso 5: Eliminar de Cancelations si estaba ahí ===
    ...
    if not reactivaciones_previas.empty:
        ...
        if pd.notnull(ultima_fecha):
            ...
            if dias_diferencia < 30:
                confirmar = input(f"⚠️ Esta línea ya fue reactivada el {ultima_fecha.date()} ({dias_diferencia} días atrás). ¿Registrar de todos modos? (s/n): ").strip().lower()
                if confirmar != "s":
...
def consultar_linea(identificador):
    ...
    for nombre_hoja, df in hojas_a_buscar:
        ...
        df = df.copy()
        df.columns = columnas  # asegura que esté en minúsculas por si acaso
        df['msisdn'] = df['msisdn'].astype(str)
        df['iccid'] = df['iccid'].astype(str)

        encontrados = df[(df['msisdn'] == identificador) | (df['iccid'] == identificador)]
        if not encontrados.empty:
            resultados[nombre_hoja] = encontrados.iloc[[-1]]  # el último que aparece en la hoja
